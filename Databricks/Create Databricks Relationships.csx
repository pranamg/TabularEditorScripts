/*
 * Title: Auto-generate relationships from Databricks
 * Author: Johnny Winter, greyskullanalytics.com
 * 
 * This script, when executed, will loop through the currently selected tables
 * and send a query to the Databricks Information Schema tables to see if any foreign keys
 * have been defined. Where foreign keys are identified, the script will create relationships
 * between the tables in the semantic model.
 * Notes: 
 ...
 *  -   The script will only create relationships where the table names and column names match exactly
 *      in Databricks and Tabular Editor. You should therefore run the script before editing any table or column names in Tabular Editor.
 */
...
- check that you have used a valid Personal Access Token",
    ...
    foreach (sysData.DataRow row in dbxRelationships.Rows)
    {
        ...
        {
            var dimTable = Model.Tables[row["pk_table_name"].ToString()];
            var dimColumn = dimTable.Columns[row["pk_column"].ToString()];
            var factColumn = t.Columns[row["fk_column"].ToString()];

            // Check whether a relationship already exists between the two columns:
            if (
                !Model.Relationships.Any(r => r.FromColumn == factColumn && r.ToColumn == dimColumn)
            )
            {
